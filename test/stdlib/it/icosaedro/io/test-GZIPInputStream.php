...
//		$gzip = new GZIPOutputStream($out, NULL, NULL, 6);
		...
		if( strlen($uncompressed) == 0 && strlen($compressed) == 0 ){
			echo "Bug https://bugs.php.net/bug.php?id=71395 still here!\n";
		}
...
//		echo "compressed: ", rawurlencode($compressed), "\n";
	...
	public function run()
	{
		...
		for($chunk_size = 1; $chunk_size < 10; $chunk_size++){
			...
			// The following test with empty string is expected to fail
			// under 7.1.0, see https://bugs.php.net/bug.php?id=71395
