...
/**
 * Instance of a matching portion of the subject string, which may contain
 * several groups of matching groups. For example, the regex "a(b)+c"
 * matches the subject string "abbbc" generating a matching element "abbbc",
 * containing one group "bbb" that contains 3 matching elements "b".
 * @access private
 */
class MatchedElement implements Element {
	private /*. string .*/ $s;
	...
	public function __toString() {
		$s = "->value() = " . Strings::toLiteral($this->value()) . "\n";
		$n = $this->count();
		for($i = 0; $i < $n; $i++){
			$gs = $this->group($i)->__toString();
			if( strlen($gs) > 0 ){
				$prefix = "->group($i)";
				$s .= $prefix . (string) str_replace("\n", "\n$prefix", trim($gs)) . "\n";
			}
		}
		return $s;
	}
...
/**
 * Instance of a matching group, which may contain zero or more matching elements.
 * For example, the group "(a|b)+" matches the subject string "abb" 3 times
 * generating a group with 3 elements "a", "b" and "b".
 * @access private
 */
 ...
 * mode, a single character can be up to 4 bytes long and this class cannot
 * be used.
 */
...
/*. private .*/ class UTF8SetMatcher extends MatcherOperator {
	...
	/*
	 * Selected chars in the range [256,0x10ffff].  Contains an even number of
 ...
 * @version $Date: 2016/02/23 10:50:13 $
 */
class Pattern implements Element {
	 ...
	 * be matched are to be assumed UTF-8 encoded.
	 * @return void
	...
	/**
	 * Returns the result of the current match. This result is "immutable" even
	 * if this pattern is used again.
	 * @return Element
	 * @throws LogicException No matching performed yet. Matching was unsuccessful
	 * then this method should not have been called.
	 */
	public function elem0(){
		if( $this->end < 0 )
	 ...
	 * @return string
	 */
	private function elementToString($e, $indent, $separator){
	...
	function resultAsString($separator = "\n"){
		return $this->elementToString($this->elem0(), "0", $separator);
	}
