 ...
 * @version $Date: 2016/02/10 19:46:57 $
 */
class Dereference {
	...
	/**
	 * Applying the "[" operator to array.
	 * @param Globals $globals
	 * @param ArrayType $a
	 * @param boolean $is_lhs If assignment can be made on this value.
	 * @return Type
	 */
	private static function dereferenceArray($globals, $a, $is_lhs)
	{
		...
		} else if( $index instanceof MixedType ){
			if( !($r->isInt() or $r->isString()) )
				$globals->logger->error($scanner->here(),
				"invalid array key of type ". $r->getType() . ", expected either int or string");
		
		} else if( !(
			$index instanceof IntType and $r->isInt()
			or $index instanceof StringType and $r->isString() )
		){
				$globals->logger->error($scanner->here(),
		...
		$scanner->readSym();
		return self::parse($globals, $a->getElem(), $is_lhs);
	}
	
	
	/**
	 * Applying the "[" operator to object implementing ArrayAccess.
	 * @param Globals $globals
	 * @param ClassType $a
	 * @return Type Type returned by the specific implemented offsetGet() method.
	 */
	private static function dereferenceArrayAccess($globals, $a)
	{
		$pkg = $globals->curr_pkg;
		$scanner = $pkg->scanner;
		...
		$globals->expect(Symbol::$sym_rsquare, "expected `]'");
		$scanner->readSym();
		$offset_get_method = $a->searchMethod( new CaseInsensitiveString("offsetGet") );
		$globals->accountMethod($offset_get_method);
		return self::parse($globals, $offset_get_method->sign->returns, TRUE);
	}
...
//				"non-static access to static property $p");
	...
	private static function evaluateInstanceOf($globals, $type)
	{
		...
		if( $scanner->sym === Symbol::$sym_x_lt )
			throw new ParseException($scanner->here(),
			"type parameters not allowed in `instanceof'");
	...
	public static function parse($globals, $type, $is_lhs)
	{
		...
		if( $sym === Symbol::$sym_lsquare ){
			if( $type instanceof StringType ){
				...
				return self::dereferenceArray($globals, cast(ArrayType::class, $type), $is_lhs);
				
			} else if( $type instanceof UnknownType ){
				return SkipUnknown::anything($globals);
				
			} else if( $type instanceof ClassType && cast(ClassType::class, $type)->isSubclassOf($globals->builtin->ArrayAccessClass)){
				return self::dereferenceArrayAccess($globals, cast(ClassType::class, $type));
				
			} else {
				$globals->logger->error($scanner->here(), "`[' operator applied to $type");
				return SkipUnknown::anything($globals);
			}
