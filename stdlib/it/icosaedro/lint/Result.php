 ...
 * @version $Date: 2016/03/01 05:06:05 $
 */
final class Result implements Printable {
	/*.
	forward public self function actualize(HashMap $replacements);
	.*/
	 ...
	 * @param string $value Common representation of the value, if available:
	 * "FALSE" and "TRUE" for boolean, 1234 if int, 3.14 if float, characters
	 * if string, "NULL" if null (also for strings, which might be a bit confusing...),
	 * "array()" for empty array, "array(...)" for non-empty array.
	 * @return Result
	 */
	...
	public function convertToString($logger, $where)
	{
		...
		} else if( $this->type->isPrintable() ){
			
			if( $this->isBoolean() ){
				$logger->notice($where,
				"implicit conversion to string of a boolean value:"
	...
	/**
	 * Evaluate weak comparison between int.
	 * @param Result $a
	 * @param string $op
	 * @param Result $b 
	...
	/**
	 * Evaluate weak comparison between floats.
	 * @param Result $a
	 * @param string $op
	 * @param Result $b 
	 * @return string Result of the comparison: "TRUE", "FALSE" or NULL if the
		...
		if( $a->isArray() ){
			...
			if( $b->isNull() ){
				...
				if($op === "==" )
					...
					$hint = " - Hint: use safer comparison expression strcmp(\$s1, \$s2) $original_op 0 instead.";
			}
		}
		
		if( ! $ok )
			$logger->error($where, "comparing (" . $this->type . ") $original_op ("
				. $other->type . ")$hint");
	...
	/**
	 * Evaluates the spaceship operator.
	 * Even knowing the values, I can't evaluate the result of the spaceship
	 * anyway because the manual only tells the sign is defined, not the
	 * value and how it get calculated internally (it may depend on the PHP
	 * version, on the platform, on the actual implementation, etc.).
	 * So, I can only return the int type without value.
	 * @param Logger $logger
	 * @param Where $where
	 * @param Result $other
	 * @return Result Int type, unknown value.
	 */
	public function spaceshipCompare($logger, $where, $other){
		if( !($this->isUnknown() || $this->isInt() || $this->isFloat()) )
			$logger->error($where, "EXPR <=> ...: only int and float types supported for the spaceship operator, found type " . $other->type);
		if( !($other->isUnknown() || $other->isInt() || $other->isFloat()) )
			$logger->error($where, "... <=> EXPR: only int and float types supported for the spaceship operator, found type " . $other->type);
		return self::$int_unknown_result;
	}
	...
	public function strongCompare($logger, $where, $other, $op){
		...
		|| ! $this->getType()->equals($other->getType()))
			// No values to compare are available or types differs.
			return self::$boolean_unknown_result;
		
		// Statically evaluate the comparison between equal types.
		if( $this->isBoolean() || $this->isString() ){
			$value = $this->value === $other->value;
		} else if( $this->isInt() || $this->isFloat() ){
			// int and float may have different representations numerically equal:
			// 1 +1 001 1.0 etc.
			// so convert to float and compare:
			$value = (float) $this->value ===  (float) $other->value;
		} else {
			return self::$boolean_unknown_result;
		}
		if( $op === "!==" )
			$value = ! $value;
		return $value? self::$true_result : self::$false_result;
	}
	 ...
	 * Note that these operators does not change the type and neither changes
	 * the value of their argument at runtime, but simply tells to PHPLint how
	 * some special values (notably, NULL and empty array) must be correctly
	 * interpreted.
	 * These type cast operators can be applied only to NULL and
	 * empty array: caller must take care to check for the current version of
	 * PHP. Programs should use the safer function cast() in any other case.
	 * @param Logger $logger
	 * @param Where $where
	...
	/**
	 * Returns a brand new actualized result out of this generic result.
	 * @param HashMap $actual_types Actual types replacing the type parameters.
	 * @return self May return $this if this result is not generic.
	 */
	public function actualize($actual_types) {
		$actualized_type = $this->type->actualize($actual_types);
		if( $actualized_type === $this->type )
			return $this;
		else
			return self::factory($actualized_type, $this->value);
	}
