 ...
 * @version $Date: 2016/02/22 08:47:35 $
 */
...
//					$logger->error($here, "constant name `" . $zzz . "' is a keyword");
		...
		if( $value->isUnknown() ){
			...
			|| $globals->isPHP(7) && $value->isArray()
			){
				if( $value->getValue() === NULL ){
					$logger->error($scanner->here(),
					"the expression giving the value of the constant is not statically determinable (PHPLint safety restriction). Hint: variables and function cannot appear in the expression. If you really need that non-constant value, define a variable instead.");
					$value = NULL;
				}
			} else {
				if( $globals->isPHP(5) )
					$allowed_types = "boolean, int, float or string";
				else
					$allowed_types = "boolean, int, float, string or array";
				$logger->error($scanner->here(), "invalid constant value of type "
				. $value->getType() . ". It must be $allowed_types");
				$value = NULL;
			}
		}
