 ...
 * @version $Date: 2016/02/22 09:37:51 $
 */
class ClassMethod implements Printable, Sortable, Hashable {
	...
	/*.
	forward 	public void function __construct(Where $where, ClassType $class_, DocBlock $docblock, Visibility $visibility, CaseInsensitiveString $name, Signature $signature);
	forward public string function __toString();
	forward public ClassMethod function actualize(HashMap $actual_types);
	forward public string function callCompatibleWithReason(ClassMethod $other);
	forward public int function compareTo(object $other) throws CastException;
	forward public boolean function equals(object $other);
	forward public boolean function equalsPrototypeOf(ClassMethod $o);
	forward public int function getHash();
	forward public string function prototype();
	forward public static void function static_init();
	pragma 'suspend';
	.*/
	 ...
	 * can be equal according to this criteria and different according to
...
//	}
	...
	public function callCompatibleWithReason($other)
	{
		...
		if( $other->is_final )
			return "final method cannot be overridden";
...
//	 * this method does not need actualization.
//	 * @var boolean
//	 */
//	private $does_not_need_actualization = FALSE;
	...
	/**
	 * Returns a brand new actualized method out of this generic method belonging
	 * to a generic class.
	 * @param HashMap $actual_types Actual types replacing the type parameters.
	 * @return self May return $this if this method is not generic or does not
	 * need actualization.
	 */
...
//			return $this;
//		}
//		// Optimization: no need to actualize private methods.
//		if( $this->visibility === Visibility::$private_ ){
//			$this->does_not_need_actualization = TRUE;
//			return $this;
//		}
		$actual_signature = $this->sign->actualize($actual_types);
//		if( $actual_signature === $this->sign ){
//			$this->does_not_need_actualization = TRUE;
//			return $this;
//		}
		$actual_method = clone $this;
		$actual_method->sign = $actual_signature;
		// FIXME: reset $used?
