	...
	static function substring($s, $start, $end)
	{
		...
		if( $s === NULL or $start < 0 or $start > $end or $end > $len )
			throw new \OutOfBoundsException("invalid range [$start,$end] for "
				. self::toLiteral($s));
		# Workaround for PHP 5 substr() bug that may return false at string boundary
		# if len==0 (see http://bugs.php.net/bug.php?id=38437 now closed).
		# Fixed in PHP 7, which now consistently returns "" in these cases. Anyway:
		if( $end - $start == 0 )
			return "";
