...
/**
	...
	@version $Date: 2015/08/26 08:19:58 $
*/
abstract class TestUnit {
	...
	/**
		...
		<p>The NULL value is rendered as NULL.
		<p>The boolean values are rendered as FALSE and TRUE.
		<p>The integer numbers are rendered in 10-base with minus sign when negative.
		<p>The floating point numbers are rendered with either a dot or the "E"
		of the scientific notation; non-finite values are rendered as NAN and INF;
		the zero negative value is rendered as -0.0.
		<p>The string of bytes are rendered as PHP literal strings of ASCII only
		characters, with non printable bytes and non ASCII bytes rendered as octal.
		<p>The arrays are rendered recursively along with their keys and values.
		<p>The objects are rendered as fully qualified class name and list of
		properties between braces. Objects implementing the {@link it\icosaedro\containers\Printable}
		...
		@param int $max_deep Maximum recursion deep on arrays and objects.
		Zero means no recursion at all; in this case arrays are always rendered
		as "array(...)" and non-Printable objects as "CLASSNAME{...}".
		@param int $max_elements Maximum number of elements to display in arrays.
		@return string Readable representation of the passed value.
	*/
